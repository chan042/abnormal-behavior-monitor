...
### 4.2 배회 탐지
목표:
- 특정 구역 내에서 목적 없는 반복 이동이 지속되는 패턴을 감지
...
- `배회`: 같은 구역 안에서 짧은 거리 이동과 방향 전환이 반복됨

1차 규칙 예시:
- 동일 ROI 내 체류시간 10분 이상
- ROI 내 왕복 횟수 4회 이상
- 방향 전환 횟수 임계치 이상
...
### 6.7 배회 규칙 엔진 구현
목표:
- ROI 내 반복 이동 패턴을 기반으로 배회 의심 이벤트를 생성한다.
...
- 배회 이벤트가 최소 한 가지 ROI 시나리오에서 탐지됨
